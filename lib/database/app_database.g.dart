...
class $FloorAppDateBase {
  ...
  static _$AppDateBaseBuilder databaseBuilder(String name) =>
      _$AppDateBaseBuilder(name);

  /// Creates a database builder for an in memory database.
  /// Information stored in an in memory database disappears when the process is killed.
  /// Once a database is built, you should keep a reference to it and re-use it.
  static _$AppDateBaseBuilder inMemoryDatabaseBuilder() =>
      _$AppDateBaseBuilder(null);
}

class _$AppDateBaseBuilder {
  _$AppDateBaseBuilder(this.name);
  ...
  _$AppDateBaseBuilder addMigrations(List<Migration> migrations) {
    _migrations.addAll(migrations);
    return this;
  }

  /// Adds a database [Callback] to the builder.
  _$AppDateBaseBuilder addCallback(Callback callback) {
    _callback = callback;
    return this;
  }
    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `NoteEntity` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `title` TEXT, `data` TEXT)');
